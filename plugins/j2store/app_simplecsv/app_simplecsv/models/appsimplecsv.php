...
class J2StoreModelAppSimplecsv extends J2StoreAppModel {
	...
	public function importFromFile() {
		...
		$importFile = $app->input->files->get( 'csvdata');
		if (empty ( $importFile ['name'] )) {
			throw new Exception ( JText::_ ( 'J2STORE_SIMPLE_CSV_ERROR_NO_FILE_SELECTED' ) );
			return false;
		}
		...
		if (! preg_match ( '#\.(' . str_replace ( array (
			...
			throw new Exception ( JText::sprintf ( 'J2STORE_SIMPLE_CSV_ERROR_ACCEPTED_TYPE', substr ( $attachment->filename, strrpos ( $attachment->filename, '.' ) + 1 ), $allowed ), 'notice' );
			return false;
		}

        if (isset($importFile ['tmp_name']) && !empty($importFile ['tmp_name'])) {
            return $this->readContent ($importFile ['tmp_name']);
        }
	...
	public function importFromPath() {
        $app = JFactory::getApplication();
        $filepath = $app->input->getString ( 'csvfile_path', '' );

		if (empty ( $filepath )) {
			throw new Exception ( JText::_ ( 'J2STORE_SIMPLE_CSV_ERROR_NO_FILE_FOUND_IN_PATH' ) );
		}
		...
		if (JFile::exists ( $path ) && JFile::getExt ( $path ) == 'csv') {
			...
			throw new Exception ( JText::_ ( 'J2STORE_SIMPLE_CSV_ERROR_READING_FILE' ) );
			return false;
		}
	...
	public function handleImport(&$contentFile) {
		...
		$app->enqueueMessage(JText::sprintf('IMPORT_REPORT',$this->totalTry,$this->totalInserted,$this->totalexits));
		return true;
	}
	...
	function _checkData(&$product,$main=false){
		...
		$content_status = $this->checkContentFieldExits($product);
		//metrics
		...
		if((!isset($product->catid) || empty($product->catid)) && $content_status){
			//$product->catid = '2';
		}

		//auto publish the article
		if((!isset($product->state) || empty($product->state)) && $content_status){
			$product->state = '1';
		}
		//access
		if((!isset($product->access) || empty($product->access)) && $content_status){
			$product->access = '1';
		}

		//language not set can't saw product view page
        if(!isset($product->language) || empty($product->language)){
            $product->language = '*';
        }
		...
		if(!isset($product->publish_up) || empty($product->publish_up)){
			...
			$product->publish_up = JFactory::getDate('now -1 day', $tz, false)->toSql(true);
		}
		...
		if(!isset($product->product_name) && isset($product->title)){
		...
		}elseif(!isset($product->sku) && isset($product->product_id) && intval($product->product_id) > 0){
			$query = $this->db->getQuery(true)->select('sku')->from('#__j2store_variants')->where('product_id='.(int) $product->product_id);
			$this->db->setQuery($query);
			$product->sku = $this->db->loadResult();
		}
	...
	public function checkContentFieldExits($product){
		...
		foreach ($fields as $field){
			if(!in_array($field, $unset_array ) && in_array($field, $keys)){
				$status = true;
			}
		}
		return $status;
			...
			if (! isset ( $columns [$this->columns [$i]] )) {
				if (isset ( $this->columnNamesConversionTable [$this->columns [$i]] )) {
					...
					if (isset ( $this->characteristicsConversionTable [$this->columns [$i]] )) {
						...
						$possibilities = array_diff ( array_keys ( $columns ), array (
								'product_id'
						) );
	...
	function _insertProducts(&$products){
		...
		/* $this->_insertOneTypeOfProducts($products,'simple');
	...
	function _insertAllProducts($products, $type="simple") {
		...
		if (! empty ( $codes )) {
			...
			if(!empty($codes)){
				foreach($products as $product){
					if($product->product_type!=$type || empty($codes[$product->sku])) continue;
	...
	public function getExportData() {
		...
		foreach ($products as $product) {
			...
			$source = isset($product->source)?$product->source:array();
