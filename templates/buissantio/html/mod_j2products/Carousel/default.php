		...
		<?php if( count($list) > 0 ):?>
			...
			<div class="product-wrap">
					...
					<?php if($product->show_title): ?>
						<h2 itemprop="name" class="woocommerce-loop-product__title">
								...
								<?php if($j2params->get('catalog_mode', 0) == 0 ): ?>
									<form action="<?php echo $product->cart_form_action; ?>"
										...
										<?php elseif( $product->product_type=='configurable' || ($cart_type == 2 && isset($product->options) && count($product->options)) || ($cart_type == 3) ):?>
											...
											<a href="<?php echo $product->module_display_link; ?>" class="j2store-button-cart <?php echo $params->get('choosebtn_class', 'sppb-btn sppb-btn-primary'); ?>"><?php echo JText::_('J2STORE_VIEW_PRODUCT_DETAILS'); ?></a>
